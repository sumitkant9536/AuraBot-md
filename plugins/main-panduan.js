...
let handler = async(m, { conn, usedPrefix, args, command }) => {
    ...
    let tulisan = `
Halo ${name}, ${ucapan()} selamat datang di menu panduan AuraBot. di menu ini kalian bisa mendapatkan panduan soal bot whatsapp.
┌〔 List Panduan 〕
├ how
├ owner
├ add
...
`.trim()
    if (!args[0]) await conn.sendButtonLoc(m.chat, await(await fetch(image)).buffer(), tulisan, wm, 'Menu', '.menu', m)
...
${deskripsi}
Query: ${search}
`.trim(), wm, 'Owner', '.owner', 'Menu', '.?', 'Rules', '.rules', m)
}
handler.tags = ['main']
...
global.panduan = [
    ...
    },
    {
        "index": "2",
        "search": "how",
        "deskripsi": "Cara menggunakan bot adalah dengan mengetikkan command bot. List command bot ada pada menu bot. jangan lupa, untuk mengakses command harus menggunakan prefix.\nPrefix AuraBot adalah all prefix.\nContoh penggunaan prefix dan command yang benar: *.menu*",
        "judul": "Cara menggunakan bot dan prefix"
    },
    {
        "index": "3",
        "search": "add",
        "deskripsi": "Cara menambahkan bot ke dalam group. silahkan baca pada menu sewa dengan mengetikkan *.sewa*. AuraBot memberlakukan trial gratis 12 jam. Setelah 12 jam maka bot akan keluar dari group. ada opsi 1 minggu, 1 bulan dan 2 bulan berlangganan.",
